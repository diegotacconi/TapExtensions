...
namespace TapExtensions.Steps.I2c.Devices
{
    public class Ads1015
    {
        ...
        */
        ...
        private static (ushort gainBits, double gainLsbSize) GetGainBits(EGainResolution gainResolutionSelection)
        {
            var gainDictionary = new Dictionary<EGainResolution, (ushort gainBits, double gainLsbSize)>
            {
                { EGainResolution.Range0, (0b0000_0000_0000_0000, 3) },
                { EGainResolution.Range1, (0b0000_0010_0000_0000, 2) },
                { EGainResolution.Range2, (0b0000_0100_0000_0000, 1) },
                { EGainResolution.Range3, (0b0000_0110_0000_0000, 0.5) },
                { EGainResolution.Range4, (0b0000_1000_0000_0000, 0.25) },
                { EGainResolution.Range5, (0b0000_1010_0000_0000, 0.125) }
            };

            if (!gainDictionary.TryGetValue(gainResolutionSelection, out var gainTuple))
                throw new ArgumentException(
                    $"{nameof(gainDictionary)} does not have an entry for " +
                    $"{nameof(gainResolutionSelection)}={gainResolutionSelection}.");

            return gainTuple;
        }

        private static ushort GetInputBits(EInputMux inputMuxSelection)
        {
            ...
            if (!inputDictionary.TryGetValue(inputMuxSelection, out var inputBits))
                throw new ArgumentException(
                    $"{nameof(inputDictionary)} does not have an entry for " +
                    $"{nameof(inputMuxSelection)}={inputMuxSelection}.");
